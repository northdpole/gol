...
void send_receive (struct data * vars) {
	...
	*/
	...
	if (mpi_size != 1) {
		...
		error = MPI_Send( &(grid[1][1]), vars->rowno, MPI_INT, vars->top, TOTOP,
             my_comm);
        if(error != 0)
			fprintf(stderr,"Error in MPI_Send\n");
        /* Copy your bottom to the bottom proccess*/
		error = MPI_Send( &(grid[vars->rowno][1]), vars->rowno, MPI_INT, vars->bot, TOBOTTOM,
             my_comm);
        if(error != 0)
			fprintf(stderr,"Error in MPI_Send\n");
      /* Copy your top left to the top left proccess*/
		error = MPI_Send( &(grid[1][1]), 1, MPI_INT, vars->t_left, TOTOPLEFT,
             my_comm);
        if(error != 0)
			fprintf(stderr,"Error in MPI_Send\n");
      
      /* Copy your top rigth to the top right proccess*/
		error = MPI_Send( &(grid[1][vars->colno]), 1, MPI_INT, vars->t_right, TOTOPRIGHT,
             my_comm);
        if(error != 0)
			fprintf(stderr,"Error in MPI_Send\n");
      
      /* Copy your bottom left to the bottom left proccess*/
		error = MPI_Send( &(grid[vars->rowno][1]), 1, MPI_INT, vars->b_left, TOBOTTOMLEFT,
             my_comm);
        if(error != 0)
			fprintf(stderr,"Error in MPI_Send\n");
      
      /* Copy your bottom right to the bottom right proccess*/
		error = MPI_Send( &(grid[vars->rowno][vars->colno]), 1, MPI_INT, vars->b_right, TOBOTTOMRIGHT,
             my_comm);
        if(error != 0)
			fprintf(stderr,"Error in MPI_Send\n");
	  ...
	  /* Copy your left to the left proccess*/
		error = MPI_Send( l_send_tmp, colno, MPI_INT, vars->left, TOLEFT,
             my_comm);
        if(error != 0)
			fprintf(stderr,"Error in MPI_Send\n");
      
      /* Copy your right to the rigth proccess*/
		error = MPI_Send( r_send_tmp, colno, MPI_INT, vars->right, TORIGHT,
             my_comm);
        if(error != 0)
			fprintf(stderr,"Error in MPI_Send\n");
        ...
        error =  MPI_Recv(&(grid[0][0]), vars->rowno, MPI_INT, vars->top, TOBOTTOM ,
        ...
        error =  MPI_Recv(&(grid[0][0]), 1, MPI_INT, vars->t_left, TOBOTTOMRIGHT ,
							my_comm, MPI_STATUS_IGNORE);
		if(error != 0)
			fprintf(stderr,"Error in MPI_Receive\n");
		/* Receive top - right process' bottom - left as your top - right shadow cell */
        error =  MPI_Recv(&(grid[0][vars->colno + 1]), 1, MPI_INT, vars->t_right, TOBOTTOMLEFT,
							my_comm, MPI_STATUS_IGNORE);
		if(error != 0)
			fprintf(stderr,"Error in MPI_Receive\n");
		/* Receive bottom - left process' top - right as your bottom - left shadow cell */
        error =  MPI_Recv(&(grid[vars->rowno + 1][0]), 1, MPI_INT, vars->b_left, TOTOPRIGHT,
							my_comm, MPI_STATUS_IGNORE);
		if(error != 0)
			fprintf(stderr,"Error in MPI_Receive\n");
		/* Receive bottom - right process' top - left as your bottom - right shadow cell */
        error =  MPI_Recv(&(grid[vars->rowno + 1][vars->colno + 1]), 1, MPI_INT, vars->b_right, TOTOPLEFT,
							my_comm, MPI_STATUS_IGNORE);
		if(error != 0)
			fprintf(stderr,"Error in MPI_Receive\n");
		/* Receive left process' right as your left shadow column */
        error =  MPI_Recv(l_rcv_tmp, colno, MPI_INT, vars->left, TORIGHT ,
							my_comm, MPI_STATUS_IGNORE);
		if(error != 0)
			fprintf(stderr,"Error in MPI_Receive\n");
		/* Receive right process' left as your right shadow column */
        error =  MPI_Recv(r_rcv_tmp, colno, MPI_INT, vars->right, TOLEFT ,
							my_comm, MPI_STATUS_IGNORE);
      ...
      for( i = 0; i< vars-> rowno; i++){
		...
		MPI_Sendrecv(grid[1], rowno+2, MPI_INT, vars->left, TOLEFT,
			grid[colno+1], rowno+2, MPI_INT, vars->right, TOLEFT,
			MPI_COMM_WORLD, &status);

		MPI_Sendrecv(grid[colno], rowno+2, MPI_INT, vars->left,
			TORIGHT, grid[0], rowno+2, MPI_INT, vars->right,
	...
	if (mpi_size == 1) {
		for (j = 0; j < rowno+2; j++) {
			grid[colno+1][j] = grid[1][j];
			grid[0][j] = grid[colno][j];
		}
	}

	// copy corners
	grid[0][0]             = grid[0][rowno];
	grid[0][rowno+1]       = grid[0][1];
	grid[colno+1][0]       = grid[colno+1][rowno];
	grid[colno+1][rowno+1] = grid[colno+1][1];

	// copy top and bottom
	for (i = 1; i <= colno; i++) {
		grid[i][0]       = grid[i][rowno];
		grid[i][rowno+1] = grid[i][1];
	}
}
